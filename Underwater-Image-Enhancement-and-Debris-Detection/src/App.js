...
function App() {
  ...
  async function fetchAPI(event) {
    ...
    fetch("/api/enhancer",{
      ...
      console.log("Finished API CALL and STATUS ",data);
      if(data.status==1){
        setPage(1)
      }
    })
  ...
  const handleAPICall=async ()=>{
    if(file!=null){
      console.log(file.name,"API CALLED");
    ...
    <div>
      ...
      <div className="header">
        <div className="inner-header flex">
          ...
          <h1>Underwater Debris Detection</h1>
          
        </div>
        {(page!=2) && <div className='uploadButtonWrapper'>
      ...
      {(page==1 ) &&<button className='backButton' onClick={handleBackClick}><IoMdArrowBack className='backIcon'/></button>}
      {(page==2) && <Load/>}
      {(page==1) && <Result/>}
    </div>
  );
